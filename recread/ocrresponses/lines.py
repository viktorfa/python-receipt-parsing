...
def find_optimal_height_factor(annotations):
    ...
    for hf in height_factors:
        results.append(
            (hf, len(get_distinct_lists(get_overlap_map(annotations, hf / 10))))
        )
        if len(results) > 1 and results[-1][1] == results[-2][1]:
            print("find_optimal_height_factor")
            print(results)
            return results[-1]
...
def get_overlap_map(annotations, height_factor=0.6):
    result = [[] for _ in annotations]
    for i, a in enumerate(annotations):
        a_y_center = get_y_center(a["bounding_poly"])
        a_height = get_y_height(a["bounding_poly"])
        for j, b in enumerate(annotations):
            b_y_center = get_y_center(b["bounding_poly"])
            b_height = get_y_height(b["bounding_poly"])
            ...
            ab_is_same_line = (
                ab_dist_center < ab_min_height * height_factor
                and ab_max_height < ab_min_height * 5
            )
...
def get_sorted_lines_by_x_axis(overlap_map, text_annotations):
    return [
        sorted(x, key=partial(overlap_group_sort, text_annotations=text_annotations))
        for x in overlap_map
    ]


def get_sorted_lines_by_y_axis(overlaps, annotations):
    return sorted(
        overlaps, key=lambda x: get_y_center(annotations[x[0]]["bounding_poly"])
    )


def overlap_group_sort(i, text_annotations):
    return min(
        text_annotations[i]["bounding_poly"]["vertices"][0]["x"],
        text_annotations[i]["bounding_poly"]["vertices"][3]["x"],
    )
