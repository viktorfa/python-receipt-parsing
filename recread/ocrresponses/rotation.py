...
from recread.ocrresponses.util import (
    get_x_length,
    get_y_length,
    get_poly_center,
    get_y_top_diff,
...
def straighten_annotations(annotations):
    angle = get_estimated_skew_angle([x["bounding_poly"] for x in annotations])
    center = (
        annotations[0]["bounding_poly"]["vertices"][0]["x"],
        annotations[0]["bounding_poly"]["vertices"][0]["y"],
    )
    for annotation in annotations:
        rotate_poly(annotation["bounding_poly"], angle, center)
...
def get_straight_poly(ocr_poly, center=(0, 0)):
    return get_rotated_poly(
        ocr_poly, find_skew_angle(ocr_poly), get_poly_center(ocr_poly)
    )
