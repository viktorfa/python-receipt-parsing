...
def is_price(match, line):
    return re.match(r'' + price_pattern + r'$', match.group()) and match.end() == len(line)


def get_match_suffix(match, line):
    unit_match = compiled_unit_pattern.match(
    ...
    if unit_match:
        return dict(
            unit=unit_match.group()[1:] if line[match.end()
                                                ] == '/' else unit_match.group(),
            value=float(match.group().replace(',', '.')),
            type='QUANTITY_UNIT_PRICE' if line[match.end(
            )] == '/' else 'QUANTITY_UNIT',
            start_pos=match.start(),
